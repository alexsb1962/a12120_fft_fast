...
//altfp_log CBX_AUTO_BLACKBOX="ALL" DEVICE_FAMILY="Cyclone III" PIPELINE=21 WIDTH_EXP=8 WIDTH_MAN=23 clock data result
...
module  log_altbarrel_shift_n3e
	( 
	aclr,
	clk_en,
	clock,
...
endmodule //log_altbarrel_shift_n3e
...
`timescale 1 ps / 1 ps
//synopsys translate_on
module  log_altbarrel_shift_vgb
	( 
	data,
	distance,
	result) ;
...
endmodule //log_altbarrel_shift_vgb
...
`timescale 1 ps / 1 ps
//synopsys translate_on
module  log_altbarrel_shift_a7e
	( 
	aclr,
	clk_en,
	clock,
...
`endif
	...
	assign
		...
		sel_w = {distance[4:0]},
		smux_w = {((({32{(sel_w[4] & (~ dir_w[4]))}} & {sbit_w[143:128], pad_w[15:0]}) | ({32{(sel_w[4] & dir_w[4])}} & {pad_w[15:0], sbit_w[159:144]})) | ({32{(~ sel_w[4])}} & sbit_w[159:128])), ((({32{(sel_w[3] & (~ dir_w[3]))}} & {sbit_w[119:96], pad_w[7:0]}) | ({32{(sel_w[3] & dir_w[3])}} & {pad_w[7:0], sbit_w[127:104]})) | ({32{(~ sel_w[3])}} & sbit_w[127:96])), ((({32{(sel_w[2] & (~ dir_w[2]))}} & {sbit_w[91:64], pad_w[3:0]}) | ({32{(sel_w[2] & dir_w[2])}} & {pad_w[3:0], sbit_w[95:68]})) | ({32{(~ sel_w[2])}} & sbit_w[95:64])), ((({32{(sel_w[1] & (~ dir_w[1]))}} & {sbit_w[61:32], pad_w[1:0]}) | ({32{(sel_w[1] & dir_w[1])}} & {pad_w[1:0], sbit_w[63:34]})) | ({32{(~ sel_w[1])}} & sbit_w[63:32])), ((({32{(sel_w[0] & (~ dir_w[0]))}} & {sbit_w[30:0], pad_w[0]}) | ({32{(sel_w[0] & dir_w[0])}} & {pad_w[0], sbit_w[31:1]})) | ({32{(~ sel_w[0])}} & sbit_w[31:0]))};
endmodule //log_altbarrel_shift_a7e
...
`timescale 1 ps / 1 ps
//synopsys translate_on
module  log_range_reduction_31e
	( 
	a0_in,
	aclr,
	almostlog,
...
endmodule //log_range_reduction_31e
...
module  log_altfp_log_via
	...
	log_altbarrel_shift_n3e   Lshiftsmall
	( 
	.aclr(aclr),
	.clk_en(clk_en),
	.clock(clock),
	...
	log_range_reduction_31e   range_reduction
	( 
	...
	.clk_en(clk_en),
	.clock(clock),
	.data({Log_normal, {17{1'b0}}}),
	.q(wire_lzc_norm_E_q));
	log_altpriority_encoder_q08   lzoc
	( 
	.data({({23{First_bit}} ^ Y0[23:1]), {9{1'b0}}}),
	.q(wire_lzoc_q));
	altsquare   squarer
	( 
	.aclr(aclr),
...
`endif
	...
	log_altfp_log_via	log_altfp_log_via_component (
				.clock (clock),
				.data (data),
				.result (sub_wire0));
